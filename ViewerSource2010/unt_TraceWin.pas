...
interface
   ...
   type
      ...
      public
         ...
         class function InternalTrace(LeftMsg: string; RightMsg: string = '')
            : PVirtualNode; overload;
         class function InternalTrace(Node: PVirtualNode; LeftMsg: string;
            RightMsg: string = ''): PVirtualNode; overload;
         class procedure InternalDumpFromThread(const memory: pointer;
            const ByteCount: Integer); static;
         class procedure checkInternals;
         function ChangeFontDetail(const IsTrace: boolean;
            const TargetCanvas: TCanvas; const Column: TColumnIndex;
            const FontDetails: TFontDetailArray; const selected: boolean)
            : boolean;
         function ChangeBackgroundColor(const TargetCanvas: TCanvas;
      ...
      public
         ...
         function CheckNode(rootVtNode: PVirtualNode; NodeToCheck: AnsiString;
            IsNew: boolean = false): PVirtualNode;
         function CheckWatch(rootVtNode: PVirtualNode; WatchName: string)
            : PVirtualNode;
         ...
         procedure SaveToTextFile(filename: string;
            SaveOptions: TSaveTofileOptions); // change the LabelLogFile caption
      public
         // detail frames
...
implementation
   ...
   class procedure TFrm_Trace.InternalDumpFromThread
      (const memory: pointer; const ByteCount: Integer);
   var
   ...
   begin
      ...
      begin
         GetStringTypeExA(LOCALE_USER_DEFAULT, CT_CTYPE1, @CurrChar, sizeof
               (AnsiChar), CurrType);
      ...
      while c <= ByteCount - 1 do begin
         ...
         while (c <= ByteCount - 1) and (d < 32) do begin
            ...
            if AnsiCharTypes[OneChar] and
               (C1_ALPHA or C1_PUNCT or C1_BLANK or C1_XDIGIT or C1_DIGIT)
               <> 0 then
   ...
   class function TFrm_Trace.InternalTrace(LeftMsg: string;
      RightMsg: string = ''): PVirtualNode;
   begin
   ...
   // create the Internal form if necesary then show it.
   // note : low level trace on text file can be done using the unt_utility.LowTrace() method
   class function TFrm_Trace.InternalTrace(Node: PVirtualNode; LeftMsg: string;
      RightMsg: string = ''): PVirtualNode;
   var
      TreeRec: PTreeRec;
   begin
   ...
   procedure TFrm_Trace.vstTraceChange(Sender: TBaseVirtualTree;
      Node: PVirtualNode);
   var
      TreeRec: PTreeRec;
               ...
               case SubMember.ViewerKind of
                  ...
                  CST_VIEWER_BITMAP: // bitmap viewer . if many bitmap, only the last will be displayed
                     begin
                        ...
                        Tframe_BitmapDetails(BitmapFrame).AddDetails
                           (TreeRec, SubMember); // add detail to frame and add frame to CurrentViewers
                     end;
                  ...
                  CST_VIEWER_TABLE:
                     begin
                        ...
                        Tframe_table(TableFrame).AddDetails
                           (TreeRec, SubMember); // add detail to frame and add frame to CurrentViewers
                     end;
                  ...
                  Tframe_Classic(TreeDetailFrame).AddDetails
                     (TreeRec, SubMember);
               end; // end case
            end; // next member
         end;
   ...
   begin
      ...
      if XMLLogFile = nil then begin
         ...
         // get the root
         XMLLogFile := doc.GetDocBinding('Data', TXMLData, TargetNamespace)
            as IXMLData;
      end;
