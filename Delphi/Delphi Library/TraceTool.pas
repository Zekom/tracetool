...
type
   ...
   public
      class procedure SendToClient (CommandList : TStringList) ; overload ;
...
begin
   ...
   MsgThread.FreeOnTerminate := false ;
   //MsgThread.Resume ;
   Resume() ;
end;
...
begin
   if MsgThread = nil then
      exit ;
   if MsgThread.CloseEvent = 0 then
      exit ;

   // ask the thread to shutdown
   setevent(MsgThread.CloseEvent) ;

   // wait for closed event (max 1 second)
   WaitForSingleObject(MsgThread.ClosedEvent, 1000) ;
   // maybee wait a few milli second before disposing the thread ?

   MsgThread.free ;       // events are closed
   MsgThread := nil ;
end;
...
begin
   ...
   setevent(MsgThread.MessageReady) ;
...
begin
   ...
   addCommand (CommandList, CST_SET_BOOKMARK,Bookmarked); // param : enabled flag
              ...
              begin
                 try  // ensure that the trace will not generate error
                    ...
                    for c := 0 to getMessageList.Count-1 do begin
                       TempObject := getMessageList.Items[c] ;
                       if not (TempObject is TStringList) then
                          continue ;
                       ...
                       if CommandList.Count >= 0 then begin
                          ...
                          if strtoint(Copy (CommandList.Strings[0],0,5)) = CST_FLUSH then begin
